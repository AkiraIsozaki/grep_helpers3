...
MASK_SPECS: dict[str, list[str]] = {
    "proc": [r'"(?:\\.|[^"\\])*"', r"'(?:\\.|[^'\\])*'", r"//[^\n]*", r"/\*.*?\*/"],
    "sql": [r"'(?:''|[^'])*'", r"--[^\n]*", r"/\*.*?\*/"],
    # shell の # は $#（引数個数）・${#var}（長さ展開）・${0##*/}（連続 #）・
    # ${arr[@]#p}（] 直後）・単語中の # を除外する
    # （誤マスクすると { が不均衡になり heuristic_span の停止判定が崩れる）。
    "shell": [r'"(?:\\.|[^"\\])*"', r"'[^']*'", r"(?<![$\{\w#\]])#[^\n]*"],
    ...
    "perl": [r"'[^']*'", r'"(?:\\.|[^"\\])*"',
             ...
             r"(?<![$@%])\bqq?![^!]*!",
             r"(?<![$@%])#[^\n]*",
             # ダイヤモンド演算子 <STDIN> <FH> <$fh> <>。マスクしないと裸 < > が
             # 分類規則の「比較」に発火し `while (<STDIN>)` が分岐に届かない。
             # レキシカルファイルハンドル <$fh> は現代 Perl の主流形なので含める。
             r"<\$?\w{0,40}>"],
    "groovy": [r"'(?:\\.|[^'\\])*'", r'"(?:\\.|[^"\\])*"', r"//[^\n]*", r"/\*.*?\*/"],
}

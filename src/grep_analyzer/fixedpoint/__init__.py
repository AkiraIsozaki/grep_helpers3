...
getter/setter は横展開せず全件 low 報告。
来歴エッジは introducers（実抽出元 Occurrence 群）ベース（偽 chain 根治）。
出力は走査順・並列完了順に非依存で決定的。

停止性: 追跡シンボルは原ソース字句のみで母集合有限。
chase_done から削らない（cap は state.capped で scan 除外のみ）ので採用集合は増える一方。
...
def run_fixedpoint(
    ...
    *, files=None, unsafe_rels=None, enc_memo=None
    ...
    ＝この経路は直接呼ぶテスト向け。本番 pipeline は常に files と unsafe_rels の両方を渡す。
    """
    ...
    # ループは lock-step 共有エンジンへ委譲（単一 keyword は byte 同値）。
    # rel_to_abs / enc_memo / Progress / automaton_split は run_fixedpoint_multi 側で駆動。

...
def test_ヒット行のSEP列がescape後もchar_maxを超えない():
    # SEP(' \n ' 4文字)は _escape_sep で5文字に膨らむ。raw 長は char_max 以下でも
    # escape 後に超過し得る。旧実装は raw 長のみで判定し最終セルが上限超過していた。
    from grep_analyzer.snippet._clamp import CHAR_MAX, SEP
    hit = SEP * 180                       # raw 720 <= 800、escape 後 900 > 800
    out = clamp_lines([hit], 0)
    assert len(out) <= CHAR_MAX

...
def test_putした内容はgetで同一に取り出せる(tmp_path):
    ...
    meta = ("int main(){}\n", "utf-8", False)   # decode 結果のみ（language/dialect は非キャッシュ・H2）
...
def test_ソース更新時はサイズmtime不一致でキャッシュミスする(tmp_path):
    ...
    cache.put(str(src), ("OLD", "utf-8", False))
    assert cache.get(str(src)) == ("OLD", "utf-8", False)
...
def test_symlink経由でもrealpath正規化で同一エントリを共有する(tmp_path):
    ...
    assert cache.get(str(link)) == meta             # symlink 経由でも decode 結果を共有（#2）

...
def test_マルチパートcp932非表現文字でもresume完了判定真(tmp_path):
    # 最高リスク経路: 非可逆エンコーディング × 複数 part。write 側 _persisted_data_rows
    # （全行 round-trip）と resume 側（part 毎読み→concat）が一致することを固定する。
    rows = [_hit("a.java", 1, "x ☃ y"), _hit("b.java", 2, "p ☃ q"),
            _hit("c.java", 3, "r ☃ s")]
    opts = _opts(output_encoding="cp932", max_rows_per_part=1)  # 3 part に分割
    finalize(tmp_path, "K", rows, opts)
    assert sum(1 for _ in tmp_path.glob("K.part*.tsv")) == 3
    assert resume.is_complete(tmp_path, "K", opts) is True

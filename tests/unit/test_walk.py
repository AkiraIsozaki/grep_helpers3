...
def test_walk_filesとcollect_files_exは9000バイト目NULで一致してbinary除外_LIVE(tmp_path):
    from grep_analyzer.walk import walk_files, collect_files_ex
    from grep_analyzer.diagnostics import Diagnostics
    # 先頭 9000 バイトは純 ASCII、その後に NUL。両経路とも 64KiB 窓で NUL を検出する
    # （#I: legacy _is_binary の 8KiB 窓を _PREFIX に統一し経路間の分岐を解消）。
    ...
    assert "edge.c" not in legacy_rels                   # 64KiB 統一: バイナリとして除外
    assert "walk_skipped_binary" in diag_legacy.render()
    ...
    assert "edge.c" not in strict_rels                   # 同一判定
    assert "walk_skipped_binary" in diag_strict.render()
...
def test_is_contained_relpathはNUL混入を拒否する():
    # 埋め込み NUL を明示拒否し、下流 is_file() の偶然の fail-close に依存しない
    # （ガード順を入れ替えても is_within_root の ValueError で落ちないため）。
    from grep_analyzer.walk import is_contained_relpath
    assert is_contained_relpath("foo\x00/etc/passwd") is False
    assert is_contained_relpath("ok/rel/path.c") is True


def test_is_within_rootはroot_realpathを毎回再計算しない(tmp_path, monkeypatch):
    # per-hit 呼出で root の realpath を都度 syscall するのは 60GB で無駄（#Q）。
    # 同一 root の連続呼出で root 側 realpath はキャッシュされ 1 回で済む。
    import os
    from grep_analyzer import walk
    root = tmp_path / "root"
    root.mkdir()
    (root / "a.c").write_text("x\n", "utf-8")
    (root / "b.c").write_text("y\n", "utf-8")
    ...
    monkeypatch.setattr(walk.os.path, "realpath", counting)
    assert walk.is_within_root(root, root / "a.c") is True
    assert walk.is_within_root(root, root / "b.c") is True
    assert root_calls["n"] == 1            # root の realpath は 1 回だけ

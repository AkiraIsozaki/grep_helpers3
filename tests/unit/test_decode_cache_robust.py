...
@pytest.mark.parametrize("bad_header", [b"null", b"5", b"[1,2]", b'"str"', b"true"])
def test_非dictヘッダのgetはAttributeErrorで落ちずmiss降格する(tmp_path, bad_header):
    # valid JSON だが dict でないヘッダ（部分破損・truncate・別ツール混在）で
    # meta.get(...) が AttributeError を投げて run を落とさないこと（H5 の堅牢化漏れ）。
    cache = DecodeCache(tmp_path / "cache")
    src = _src(tmp_path, "a.c", b"int main(){}\n")
    cache.put(str(src), ("int main(){}\n", "utf-8", False, "c", "bourne"))
    dca = next((tmp_path / "cache").glob("*.dca"))
    raw = dca.read_bytes()
    nl = raw.find(b"\n")
    dca.write_bytes(bad_header + b"\n" + raw[nl + 1:])
    assert cache.get(str(src)) is None       # AttributeError で落ちない

...
def test_Javaのif比較行は比較highと判定する():
    src = 'class A {\n void m(){\n  if (s.equals("K")) {}\n }\n}\n'
    assert classify_ast("java", src, 3) == ("比較", "high")


def test_Javaのフィールド宣言は宣言highと判定する():
    src = 'class A {\n static final String K = "K";\n}\n'
    assert classify_ast("java", src, 2) == ("宣言", "high")


def test_Cのdefineは宣言highと判定する():
    src = '#define CODE "X"\nint main(){return 0;}\n'
    assert classify_ast("c", src, 1) == ("宣言", "high")
...
def test_ProCはEXEC_SQLをマスクしてもC宣言を分類できる():
    ...
    src = 'int f(){\n EXEC SQL SELECT 1;\n char *p = "X";\n}\n'
    assert classify_ast("proc", src, 3) == ("宣言", "high")
...
def test_javascript_category():
    assert classify_ast("javascript", "const X = 1;\n", 1) == ("宣言", "high")
    assert classify_ast("javascript", "a = b;\n", 1) == ("代入", "high")
    assert classify_ast("javascript", "switch (x) { case 1: break; }\n", 1) == ("分岐", "high")


def test_typescript_category():
    assert classify_ast("typescript", "enum E { A, B }\n", 1) == ("宣言", "high")
    assert classify_ast("typescript", "interface I { x: number; }\n", 1) == ("宣言", "high")


def test_tsx_category():
    assert classify_ast("tsx", "const App = () => <div>{x}</div>;\n", 1) == ("宣言", "high")
...
def test_コメント行はコメントlowと判定する_各AST言語():
    ...
    assert classify_ast("java", "class A {\n /* 777 */\n}\n", 2) == ("コメント", "low")
    assert classify_ast("c", "int x;\n// 777\n", 2) == ("コメント", "low")
    assert classify_ast("c", "int x;\n/* 777 */\n", 2) == ("コメント", "low")
    assert classify_ast("python", "# 777\nx = 1\n", 1) == ("コメント", "low")
    assert classify_ast("javascript", "// 777\nconst x = 1;\n", 1) == ("コメント", "low")
    assert classify_ast("typescript", "// 777\nconst x = 1;\n", 1) == ("コメント", "low")


def test_文ブロック内ネストコメントもコメントになる():
    src = "class A {\n void m(int x){\n  if (x == 1) {\n   // 777\n  }\n }\n}\n"
    assert classify_ast("java", src, 4) == ("コメント", "low")
